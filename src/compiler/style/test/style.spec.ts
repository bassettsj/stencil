...
describe('component-styles', () => {
  ...
  describe('build', () => {
    ...
    it('should build 2 bundles w/ 3 components w/ styleUrls', async () => {
      c.config.bundles = [
      ...
      expect(cmpA.includes('body { color: red; }')).toBe(true);
      expect(cmpA.includes('body { color: white; }')).toBe(true);

      const cmpC = await c.fs.readFile('/www/build/app/cmp-c.js');
      expect(cmpC.includes('body { color: blue; }')).toBe(true);
    });
